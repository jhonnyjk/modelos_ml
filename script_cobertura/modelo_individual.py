...
def clasificar_rotacion_y_demanda(dataset_path):
    ...
    for sku in skus:
        ...
        proporciones_meses_con_consumo = meses_con_consumo / 36

        # Lógica de Clasificación
        if proporciones_meses_con_consumo >= 0.75:
            clasificacion['SKU'] = 'Alta Rotación'
        elif 0.25 <= proporciones_meses_con_consumo < 0.75:
            clasificacion['SKU'] = 'Baja Rotación'
        else:
            clasificacion['SKU'] = 'Uso Inmediato'
        ...
        adi = 36 / meses_con_consumo if meses_con_consumo > 0 else np.nan

        # Lógica de Tipo de Demanda
        if adi < 1.32 and cov_squared < 0.49:
            demanda_tipo['SKU'] = 'Suavizado (Smooth demand)'
        elif adi >= 1.32 and cov_squared < 0.49:
            demanda_tipo['SKU'] = 'Intermitente (Intermittent demand)'
        elif adi < 1.32 and cov_squared >= 0.49:
            demanda_tipo['SKU'] = 'Errática (Erratic demand)'
        elif adi >= 1.32 and cov_squared >= 0.49:
            demanda_tipo['SKU'] = 'Grumosa (Lumpy demand)'

        # Crear un DataFrame con los resultados para el SKU actual
        resultados_sku = pd.DataFrame({
            'SKU': [clasificacion['SKU']],
            'Clasificación': [clasificacion['SKU']],
            'Proporción Meses con Consumo': [proporciones_meses_con_consumo],
            'Coeficiente de Variación (CV)': [cv],
            'ADI (Average Demand Interval)': [adi],
            'Tipo de Demanda': [demanda_tipo['SKU']]
        })
    ...
    export_path = os.path.join(os.path.dirname(dataset_path), 'resultados_demanda_por_sku_fijo.xlsx')
